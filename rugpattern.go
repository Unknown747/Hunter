...
const (
	...
	rugMatchThreshold  = 3              // butuh 3+ rug serupa dalam window untuk blokir
	rugSimilarityNeeds = 3             // butuh 3 dari 4 dimensi cocok → dianggap mirip
)
...
func (rs *RugPatternStore) Check(t *TokenInfo) (bool, string) {
	...
	for _, p := range rs.patterns {
		...
		// Dimensi 1: umur pair serupa (±15 menit)
		if p.AgeMinutes > 0 && math.Abs(p.AgeMinutes-ageMin) < 15 {
			score++
		}

		// Dimensi 2: buy ratio saat entry serupa (±0.10)
		if math.Abs(p.BuyRatioEntry-t.BuyRatio) < 0.10 {
			score++
		}

		// Dimensi 3: price pump 5m saat entry serupa (±15%)
		if math.Abs(p.PricePump5m-t.PriceChange5m) < 15 {
			score++
		}

		// Dimensi 4: likuiditas dalam range 3x
