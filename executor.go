...
func (e *LiveExecutor) Buy(t *TokenInfo, _ float64) (Fill, error) {
	...
	tradeUSD := weiToFloat64(e.tradeSizeWei) * ethPriceUSD(e.client)
	return Fill{
		Action:    "BUY",
		Price:     t.Price,
		PctOfPos:  100,
...
// ethPriceUSD queries WETH/USDC price via getAmountsOut to get a live ETH price.
// Falls back to $3000 if the query fails.
func ethPriceUSD(client *ethclient.Client) float64 {
	rABI, err := abi.JSON(strings.NewReader(abiRouter))
	if err != nil {
		return 3000
	}
	...
	data, err := rABI.Pack("getAmountsOut", oneETH, routes)
	if err != nil {
		return 3000
	}
	...
	result, err := client.CallContract(ctx, geth.CallMsg{To: &routerAddr, Data: data}, nil)
	if err != nil || len(result) == 0 {
		return 3000
	}

	out, err := rABI.Unpack("getAmountsOut", result)
	if err != nil {
		return 3000
	}
	amounts, ok := out[0].([]*big.Int)
