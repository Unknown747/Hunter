...
type TokenState struct {
        ...
        LastVolume    float64
        LastPrice     float64
        FirstVolume   float64 // volume at first observation (for spike calculation)
        FirstSeen     time.Time
        LastSeen      time.Time
...
type Fill struct {
        ...
        Price     float64   `json:"price"`
        PctOfPos  float64   `json:"pctOfPos"` // % of original position
...
type StrategyConfig struct {
        ...
        MaxPricePump5m  float64 // max 5-min price pump to avoid chasing
        // Exit conditions
        TP1Pct       float64 // profit % to trigger TP1
        TP1SellFrac  float64 // fraction of position to sell at TP1
        TP2Pct       float64 // profit % to trigger TP2 (close rest)
        StopLossPct  float64 // loss % to stop out (negative)
        EmergencyBuyRatio    float64 // buy ratio below this → emergency exit
        SuddenDumpThreshold  float64 // 5m price change below this → emergency exit
        VolumeDropFraction   float64 // vol < entryVol * this → emergency exit
        MaxHoldMinutes       float64
        MinProfitForHold     float64 // if hold > MaxHoldMinutes AND profit < this → exit
        // Position sizing
        TradeSizeUSD  float64
        MaxOpenTrades int
}

// DefaultConfig returns the strategy as specified.
