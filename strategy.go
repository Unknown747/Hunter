...
func CheckEntry(t *TokenInfo, state *TokenState, cfg *StrategyConfig) EntryResult {
        ...
        // ── Proteksi ekstra untuk token sangat baru (< 10 menit) ──────────────────
        // Fase ini paling rawan manipulasi — naikkan threshold entry secara otomatis.
        // Jika sudah lewat 10 menit, pakai threshold normal dari konfigurasi.
        if ageMin < 10 {
                if t.BuyRatio < 0.72 {
                        return EntryResult{false, fmt.Sprintf("early-guard: token %.0fm buyRatio %.2f < 0.72 (threshold ketat)", ageMin, t.BuyRatio)}
                }
                if t.PriceChange5m > 40 {
                        return EntryResult{false, fmt.Sprintf("early-guard: token %.0fm pump 5m +%.1f%% > 40%% (terlalu cepat)", ageMin, t.PriceChange5m)}
                }
        }
        ...
        // ── Volume relatif terhadap likuiditas ────────────────────────────────────
        // Token dengan volume sangat rendah dibanding pool size = tidak ada minat nyata.
        // Likuiditas bisa di-seed besar tanpa ada trading — ini bukan sinyal bagus.
        if t.Liquidity > 0 && ageMin > 15 {
                volLiqRatio := t.Volume24h / t.Liquidity
                if volLiqRatio < 0.08 {
                        return EntryResult{false, fmt.Sprintf("vol/liq=%.3f < 0.08 (aktivitas trading sangat rendah)", volLiqRatio)}
                }
        }
