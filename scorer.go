...
func Score(t *TokenInfo) float64 {
        ...
        // Bonus umur — terdiferensiasi per window validasi:
        // < 3 menit   : TIDAK dapat bonus (terlalu baru, tidak ada validasi pasar)
        // 3–15 menit  : sweet spot early entry, konfirmasi awal ada
        // 15–30 menit : masih early, aktivitas sudah mulai terkonfirmasi
        // 30–60 menit : bonus kecil, window hunting masih relevan
        // > 60 menit  : tidak ada bonus umur
        if t.PairAgeHours > 0 {
                switch {
                case ageMin >= 3 && ageMin < 15:
                        score += 10.0 // early entry terbaik
                case ageMin >= 15 && ageMin < 30:
                        score += 7.0 // masih muda, terkonfirmasi
                case ageMin >= 30 && ageMin < 60:
                        score += 3.0 // bonus kecil — masih dalam window
                }
        }
        ...
        // Bonus partisipasi nyata: banyak transaksi dengan buy dominan
        if totalTxns >= 50 && buyRatio > 0.60 {
                score += 4.0
        }

        // ── Penalties ─────────────────────────────────────────────────────────────

        // Penalty: terlalu baru (< 3 menit) — risiko rug/manipulasi awal sangat tinggi
        if t.PairAgeHours > 0 && ageMin < 3 {
                score -= 10.0
        }
        ...
        // Penalty: sell pressure mulai dominan tapi belum extreme (peringatan dini)
        if totalTxns > 20 {
